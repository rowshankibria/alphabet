...
### A1. Fix the GrayOut color (line 609)

The CSS `.GrayOut` is `silver`. Change the `GrayOutBg` value from `#808080` to:
...
Paste just before `</ReportParameters>` (line 689):
...
Only the two color bindings change. Priority: **GrayOut** (ActivityIdentifier = N, or empty date), then **the code from C#**, then **Job**.

#### `txtTOIssued`: replace lines 284–295
...
namespace FMS_PDFReportService.Engine.Colors
{
    ...
    public static class FmsColorCalculator
    {
        ...
        // Empty date = null or the 12/31 23:59 placeholder (same test the .trdx uses)
        public static bool IsEmpty(DateTime? d)
        {
            return d == null || (d.Value.Month == 12 && d.Value.Day == 31 && d.Value.Hour == 23 && d.Value.Minute == 59);
        }
        public static bool HasValue(DateTime? d) { return !IsEmpty(d); }
        ...
        public static void Apply(IList<FmsJob> jobs, FmsColorInputs inp)
        {
            ...
            foreach (var j in jobs)
            {
                ...
                for (int m = fodMoves.Count - 1; m >= 0; m--)
                {
                    ...
                    if (f.TORS == FmsConstants.Job_TakeOutInt && HasValue(j.TOIssued) && IsEmpty(j.TOComplete))
                    ...
                    if (f.TORS == FmsConstants.Job_RestoreInt && HasValue(j.RSIssued) && IsEmpty(j.RSComplete))
                ...
                if (inp.FaultMoves.Any(x => x.Job == j.Job) && HasValue(j.TOIssued) && IsEmpty(j.TOComplete))
                ...
                if (IsEmpty(j.TOIssued))   j.TOIssuedBG   = FmsConstants.CC_PALE_GRAY;
                if (IsEmpty(j.TOComplete)) j.TOCompleteBG = FmsConstants.CC_PALE_GRAY;
                if (IsEmpty(j.RSIssued))   j.RSIssuedBG   = FmsConstants.CC_PALE_GRAY;
                if (IsEmpty(j.RSComplete)) j.RSCompleteBG = FmsConstants.CC_PALE_GRAY;
                ...
                if (HasValue(j.TOIssued) && ToInt(j.TOOperator) == FmsConstants.Operator_FCR)
                {
                    ...
                    if (permitStatus != -1)
                    {
                        PickColor(inp, FmsConstants.Operator_FCR, j.Job, permitStatus,
                                  ...
                                  permitStatus == FmsConstants.Permit_Initialized && IsEmpty(j.RSComplete),
                                  out bg, out fg);

                        if (IsEmpty(j.RSComplete) && HasValue(j.TOIssued) && FcrIssuedStatuses.Contains(permitStatus))
        ...
        private static void ApplyOpOrder(FmsJob j, FmsColorInputs inp, int tors)
        {
            ...
            if (active == null)
            {
                ...
                if (rows.Count == 0)
                {
                    if (tors == FmsConstants.Job_TakeOutInt && IsEmpty(j.TOComplete)) j.TOIssuedFG = FmsConstants.CC_RED;
                    if (tors == FmsConstants.Job_RestoreInt && IsEmpty(j.RSComplete)) j.RSIssuedFG = FmsConstants.CC_RED;
                }
                return;
            }
            ...
            {
                if (HasValue(j.TOIssued) && IsEmpty(j.TOComplete))                          { j.TOIssuedFG = fg;   j.TOIssuedBG = bg; }
                if (IsEmpty(j.TOComplete) && IsEmpty(j.RSIssued) && HasValue(j.RSComplete)) { j.RSCompleteFG = fg; j.RSCompleteBG = bg; }
                if (HasValue(j.TOComplete))                                                  { j.TOCompleteFG = fg; j.TOCompleteBG = bg; }
            }
            else
            {
                if (HasValue(j.RSIssued) && IsEmpty(j.RSComplete)) { j.RSIssuedFG = fg;   j.RSIssuedBG = bg; }
...
| A cell colored when its date is blank, or the reverse | `IsEmpty()` treats `null` and `12/31 23:59` as empty, the same test as the .trdx. OpenROAD tells `NULL` apart from `''`; C# can't, so a few edge rows could differ. |
