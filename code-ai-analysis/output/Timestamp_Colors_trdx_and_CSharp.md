...
## Audit — every SetFMSColors condition and where it is in C#

`ca.dmt` line → the C# code in B2 (`FmsColorCalculator`). NULL = `IsNull`, `''` = `IsBlank`, "IS NOT NULL AND <> ''" = `HasValue`.

| ca.dmt | OpenROAD condition | C# |
|---|---|---|
| 105043–105046 | All 4 FG = BLACK (BG not touched, starts at 0) | Defaults block (all FG = 1, all BG = 0) |
| 105069–105095 | Alert: `AlertTime <> ''` and `< now` → RED; `AlertTime <> ''` and `InitialAlert = FALSE` → YELLOW; else Acknowledged → PALE_GREEN; else BG 0 / FG PALE_GRAY | `PickColor` (`hasAlert = HasValue(AlertTime)`, `InitialAlert == 0`) |
| 105067–105068 | FOD rows checked last-to-first, each used row removed | `for (m = Count-1 … 0)` + `RemoveAt` |
| 105097–105100 | FOD TO: TOIssued HasValue AND TOComplete IS NULL → TOIssued | `HasValue(TOIssued) && IsNull(TOComplete)` |
| 105107–105109 | FOD TO: TOComplete HasValue → TOComplete | `HasValue(TOComplete)` |
| 105115–105118 | FOD RS: RSIssued HasValue AND RSComplete IS NULL → RSIssued | `HasValue(RSIssued) && IsNull(RSComplete)` |
| 105124–105126 | FOD RS: RSComplete HasValue → RSComplete | `HasValue(RSComplete)` |
| 105138–105144 | Fault: TOIssued HasValue AND TOComplete IS NULL → TOIssuedFG PALE_GRAY | `HasValue(TOIssued) && IsNull(TOComplete)` |
| 105150–105162 | TOOperator = 10001 → order with status IN (10,15,20,50), TO side | `ApplyOpOrder(…, TakeOut)`, `active` row |
| 105166–105182 | No order at all AND TOComplete IS NULL → TOIssuedFG RED | `rows.Count == 0` + `IsNull(TOComplete)` |
| 105213–105219 | TOIssued HasValue AND (TOComplete IS NULL OR = '') → TOIssued | `HasValue(TOIssued) && (IsNull(TOComplete) \|\| IsBlank(TOComplete))` |
| 105223–105229 | TOComplete = '' AND RSIssued = '' AND RSComplete not NULL → RSComplete | `IsBlank(TOComplete) && IsBlank(RSIssued) && !IsNull(RSComplete)` |
| 105231–105235 | TOComplete HasValue → TOComplete | `HasValue(TOComplete)` |
| 105242–105275 | RSOperator = 10001, RS side; no order AND RSComplete IS NULL → RSIssuedFG RED | `ApplyOpOrder(…, Restore)` + `IsNull(RSComplete)` |
| 105306–105311 | RSIssued HasValue AND RSComplete IS NULL → RSIssued | `HasValue(RSIssued) && IsNull(RSComplete)` |
| 105313–105317 | RSComplete HasValue → RSComplete | `HasValue(RSComplete)` |
| 105321–105335 | Each date = '' → BG PALE_GRAY | `IsBlank(…)` (NULL is **not** grayed) |
| 105337–105342 | ActivityIdentifier = 'N' → all 4 BG PALE_GRAY | `ActivityIdentifier == "N"` |
| 105347–105348 | TOIssued IS NOT NULL AND TOOperator = 10002 | `!IsNull(TOIssued) && … == Operator_FCR` |
| 105350–105357 | Permit status for this job; none → no color | `PermitStatus = -1` → skipped |
| 105377–105387 | Acknowledged → PALE_GREEN; Initialized AND RSComplete IS NULL → PALE_RED; else BG 0 / FG PALE_GRAY | `PickColor(ack, Initialized && IsNull(RSComplete))` |
| 105397–105407 | RSComplete IS NULL AND TOIssued HasValue AND status IN (10,20,12,15,0) → TOIssued | `IsNull(RSComplete) && HasValue(TOIssued) && FcrIssuedStatuses` |
| 105409–105411 | ELSE status = 50 → RSComplete | `else if (Permit_Returned)` |
| 105420–105428 | Process Automation → Issued BG LIGHT_BROWN (TO or RS) | PA loop |
| 105050–105062 | 33kv ResponseBG | **Not ported**: `fcardjs.txt` never reads ResponseBG |

### Open items (cannot be settled from the files)

1. **Exact value of an Ingres `''` date in C#.** The code and the `.trdx` treat any date at 12/31 23:59 as `''`, the same test your original report used. A **real** event at exactly 12/31 23:59 would be shown as not used (gray). Look at the value in the debugger (e.g. `9999-12-31 23:59:59`). If it has a fixed year, add the year to `IsBlank` in `FmsColors.cs` and change the 4 bindings to `Format("{0:yyyy/MM/dd HH:mm}", …) = "<that value>"`.
2. **Several matching rows.** Where OpenROAD's single-row SELECT finds more than one row (two alerts or two active orders for the same move), which row it takes isn't defined in the code. C# takes the first row returned.
3. **Initial BG value.** OpenROAD doesn't reset the BG codes before the rules run; C# starts them at 0. This assumes the job list is freshly loaded, as it is on the web screen.
